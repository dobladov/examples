...
fn create_sub_window(title: &str, main_window_entry: gtk::Entry, id: usize,
                     windows: Rc<RefCell<HashMap<usize, gtk::Window>>>) -> gtk::Window {
...
fn generate_new_id(windows: Rc<RefCell<HashMap<usize, gtk::Window>>>) -> usize {
    ...
    while windows.borrow().get(&len).is_some() {
        len += 1;
    }
    len
}
...
fn main() {
    ...
    button.connect_clicked(move |_| {
        ...
        let new_window = create_sub_window(&windows_title_entry_clone.get_buffer().get_text(),
                                           main_entry.clone(),
                                           new_id,
                                           windows.clone());
        // Once the new window has been created, we put it into our hashmap so we can update its
        // title when needed.
        windows.borrow_mut().insert(new_id, new_window);
    });
